...
| OQ-Rigor | The derived ontology attains IOF-class DEFINITIONAL RIGOR: `definitional_completeness ≥ 0.45` AND `realizable_machinery > 0` (BFO role/disposition/function discipline, not rigid subclasses), measured by `scripts/ontology_metrology.py` against the IOF/BFO signature | the OQuaRE Functional-Adequacy floor (≥3.0) in `sync._gate`; the v0.4+ ontology Data Product release | **SUPPORTED — gate 🟢 GREEN (2026-06-29)** — a prompt nudge failed (def_complete 0.015, gate RED), so the GEPA-style **rigor-evolution** closed it (`scripts/evolve_rigor.py` + `aegir.ontology.ontoclean`): the OntoClean membrane FLAGS ≡-candidates (anti-rigid∧relational→role; genus+differentia kind→≡), the engine DECIDES sufficiency (kept ~21 primitive — a real judgment, not blind conversion), a deterministic SubClassOf→EquivalentTo rewrite + HermiT dispose. def_complete 0.015→0.302 (62 ≡), 0 unsat. **Free authoring (2026-06-29, E1/E2):** a per-axiom validation membrane (`evolve_rigor.validate_axioms`) admits the agent's WELL-FORMED free-form axioms (refined ≡ + BFO roles bearing realizes/inheres + NEW {Bearer}/{Process} slots the templates never had) — the agent TRANSCENDS the slot DSL, bounded by parse+HermiT+OntoClean. Result: def_complete→**0.349** (77 ≡), **realizable 0→10 (`>0` MET via 5 free-authored roles)**, FunctionalAdequacy 2.37→**4.26**, OQuaRE 2.57→**4.48**, 0 unsat. **def_complete `≥0.45` MET (0.349→0.551, 133 ≡), 2026-06-29 (T2)** via the agent-mediated FEEDBACK LOOP defining the referenced fillers (`define_fillers.py` — propose → the membrane returns its REASON → re-prompt the rejects WITH their failure → resubmit; 56 filler ≡ across 2 rounds, round-2 recovered 11 of 25 rejects, demonstrating the agent RESPONDING to the gate). **BOTH OQ-Rigor strict targets now MET** (def_complete 0.551 ∧ realizable 10); OQuaRE 4.18 GREEN, 0 unsat. **Grounding capstone (G-1, 2026-06-29):** CCO+FHIR+accretive grounding-anchor retrieval (`grounding_anchors.py`) + CCO imported as a REASONING authority (`build_realized_ontology.consistency_check` — HermiT validates grounding against CCO's 26 disjointness axioms IN the two-membrane `define_intermediate_classes.py` loop). The renamed loop (intermediate classes ≠ "fillers" — they are the property-bearing CTA subsumers) grounds 88% of genera to real CCO/sdg classes (0 circular; differentia 0 generic-BFO); def_complete **0.554**, bfo_grounded 0.87→**0.896**, 0 unsat (in-loop HermiT ∧ Phase-A realize AGREE), OQuaRE **4.24** GREEN. The agent now responds to the REASONER in-loop, not just the parser. |
...
| DA-1 | *(domain-adaptivity / cold-start)* the BFO/CCO/FHIR foundation, extended with **SysMLv2**, is domain-adaptive: a clean content-first cold-start (NO seed templates) produces a metrology-validated, TWO-domain (LIMS + manufacturing) ontology + overlapping corpus from bulk FinePDFs alone, qualifying the whole pipeline end-to-end | the substantive v0.4 `sdg-corpora` release; the full bulk run (itself gated by the small qualification run) | **UNTESTED — pre-registered 2026-06-29.** Decision rule (pre-registered): QUALIFIED iff OQuaRE GREEN (aggregate ≥3.5) ∧ 0 unsatisfiable (HermiT) ∧ corpus 4-scorer verify PASS ∧ BOTH domains present in the realized ontology ∧ EPL-clean (SysMLv2 drawn+cited, no EPL files in the artifact). The small qualification run must pass before full bulk. Seed archived as `seed-baseline-v0.3`. |
