...
_reflog = _logging.getLogger("aegir.lineup.reference_integrity")
        ...
        if loc and loc in label_pref:
            ...
            if arch:
                ...
                _reflog.error("reference integrity: %(panel)s → %(reference)r resolves nowhere "
                              "(iri=%(iri)r, attempted=%(attempted)s)", d)
...
def project_vocab_concepts(pts: "list[dict]", live_ids: "set | None" = None,
    ...
    try:
        ...
        return []
    live_ids = live_ids or set()
    # JOIN BY LABEL: payload "codes" are counter notations that RENUMBER across vocab
