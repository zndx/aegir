...
`part def` denotes a physical part → cco:Artifact (not "the SysML PartDefinition model element").
...
BRIDGE: dict[str, tuple[str, str, str]] = {
    "Item": ("bfo:continuant", "a thing (continuant) that can flow through, occupy, or compose a system", "core"),
    "Part": ("cco:Artifact", "a designed physical part or component of a system — a material artifact", "manufacturing"),
    "Port": ("bfo:disposition", "a connection point of a part — a disposition to interface with another part", "manufacturing"),
    "Interface": ("bfo:disposition", "a defined interaction capability realized between connected parts", "manufacturing"),
    "Connection": ("bfo:relational_quality", "a structural connection holding between parts of a system", "manufacturing"),
    "Action": ("bfo:process", "an activity or behavior a system performs — a process", "core"),
    "StateAction": ("bfo:process", "a stative condition a system maintains over an interval", "core"),
    "RequirementCheck": ("cco:DirectiveICE", "a required condition or capability a system shall satisfy", "engineering"),
    "ConstraintCheck": ("cco:ICE", "a constraint expression asserted over system properties", "engineering"),
    "AnalysisCase": ("cco:ICE", "an analysis specification evaluated against a system model", "engineering"),
    "ShapeItem": ("bfo:spatial_region", "a geometric shape occupying a spatial region (incl. CSG solids)", "geometry"),
    "SpatialItem": ("bfo:spatial_region", "a spatially-extended item with position and orientation", "geometry"),
    "ScalarQuantityValue": ("cco:Measurement", "a measured scalar quantity (ISQ) borne by an entity", "measurement"),
    "QuantityValue": ("cco:Measurement", "a measurable quantity (ISQ) borne by an entity", "measurement"),
    "MeasurementUnit": ("cco:MeasurementUnit", "a unit of measure (SI/ISQ) for a quantity", "measurement"),
    "UnitOfMeasure": ("cco:MeasurementUnit", "a unit of measure (SI/ISQ) for a quantity", "measurement"),
}

# Per-file default genus (inheritance fallback for concepts not named in BRIDGE).
FILE_GENUS: dict[str, tuple[str, str]] = {
    "Items": ("bfo:continuant", "core"),
    "Parts": ("cco:Artifact", "manufacturing"),
    "Ports": ("bfo:disposition", "manufacturing"),
    "Connections": ("bfo:relational_quality", "manufacturing"),
    "Interfaces": ("bfo:disposition", "manufacturing"),
    "Actions": ("bfo:process", "core"),
    "States": ("bfo:process", "core"),
    "Requirements": ("cco:DirectiveICE", "engineering"),
    "Constraints": ("cco:ICE", "engineering"),
    "AnalysisCases": ("cco:ICE", "engineering"),
    "Allocations": ("cco:ICE", "engineering"),
    "ShapeItems": ("bfo:spatial_region", "geometry"),
    "SpatialItems": ("bfo:spatial_region", "geometry"),
    "Quantities": ("cco:Measurement", "measurement"),
    "ISQ": ("cco:Measurement", "measurement"),
    "SI": ("cco:MeasurementUnit", "measurement"),
}
...
def ground(concept: dict, file_stem: str) -> dict:
    ...
    if name in BRIDGE:
        ...
        genus, domain = FILE_GENUS.get(file_stem, ("bfo:continuant", "core"))
