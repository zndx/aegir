#!/usr/bin/env python
"""Realize the greenfield entity-derived ontology: merge → HermiT certify → DDL/shapes → score.
...
holds: kvasir fast-refutes in-loop, **HermiT signs the certificate** here.
  ...
  build/greenfield/<run>/entities/*.json
    → merge_entities → sdg-greenfield.omn        (the real ontology, HermiT/DeepOnto-feedable)
...
Run: LD_LIBRARY_PATH=$(pwd)/build/jvm-libs uv run --no-sync python scripts/realize_greenfield.py \
...
def realize(entities_dir: Path, output_dir: Path, *, skip_hermit: bool = False) -> dict:
    ...
    omn_path = output_dir / "sdg-greenfield.omn"
