...
# then deploys metaflow-service + UI into the aegir-metaflow namespace, pointed at the devenv postgres and
# Signals' RustFS (the S3 datastore, host :9010).
set -euo pipefail
...
# The datastore the pods will dial must exist before we point them at it.
require_rustfs

# Render templates with the runtime pg port + host IP into build/metaflow/ (gitignored).
RENDER_DIR="$REPO/build/metaflow"
mkdir -p "$RENDER_DIR"
export PGPORT AEGIR_HOST_IP AEGIR_RUSTFS_PORT AEGIR_RUSTFS_ACCESS_KEY AEGIR_RUSTFS_SECRET_KEY AEGIR_MF_NAMESPACE
...
echo "  rendered Endpoints+values for pg :$PGPORT, rustfs :$AEGIR_RUSTFS_PORT, host $AEGIR_HOST_IP"
