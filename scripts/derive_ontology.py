...
_SYSTEM = (
    ...
    "{X:Class}/{Y:Class} as SLOTS (never fill a class slot with an IRI), but RENAME every class slot to a "
    "domain-meaningful CamelCase concept via class_slot_names (class-slot-token -> ConceptName, e.g. "
    "{\"X\":\"AblationProcess\",\"Y\":\"TissueRegion\"}) so the axiom, prose and columns read in domain terms, "
    "never bare {X}/{Y}. Fill ONLY property/data slots and $DESIGN params, keying property_fillers "
    "by the EXACT slot name shown before the colon (e.g. for {p:ObjectProperty} use key \"p\", value "
    "\"sdg:hasAnalyte\"; never key a class slot in property_fillers, never invent keys). Coin camelCase sdg: IRIs. (2) anchor "
    ...
    "the catalog), template_id (snake_case of the PRIMARY subject concept, e.g. video_analytics_execution — "
    "NEVER the pattern name), class_slot_names (class-slot-token -> domain ConceptName, one per "
    "class slot in the chosen pattern), property_fillers (slot-name -> sdg:IRI), new_properties (list of "
    "{iri,label,domain,range}), bfo_anchor_path (list of prefixed BFO/CCO IRIs, leaf last), verbal_template "
    "(one sentence using the renamed {slots}), source_span (a short quote from the passage that motivated it), "
