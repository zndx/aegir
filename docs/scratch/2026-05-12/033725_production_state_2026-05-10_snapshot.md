...
### The closed-loop ontology-to-corpus pipeline

The core of Ægir's training pipeline is a closed-loop
ontology-to-corpus generation process that converts high-quality
organic text into a perfect-ground-truth reinforcement-learning
corpus. As illustrated in **Figure 3.1**, input corpora are first
mapped to concepts expressed in a BFO/CCO-grounded intermediate
representation (the SDG catalog of 540 templates). These concepts
feed procedural verbalization via the DeepOnto recursive verbaliser,
producing natural-language descriptions that an LLM then uses to
generate output corpora. Topic preservation — verified through
BERTopic alignment between input and output corpora — serves as
the *R_D* component of the deterministic verifier *R(O, I)*,
ensuring content fidelity. The resulting synthetic relational
tables and verbalized ontology compositions become the held-out
RL corpus, enabling the model to learn structured-output
generation under a verifiable scalar reward.
...
**Figure 3.1 — Ægir closed-loop synthetic data generation
pipeline.** The pipeline transforms raw input corpora into output
corpora via a BFO/CCO-grounded intermediate representation of
structured concepts. Procedural verbalization (DeepOnto) followed
by LLM generation produces the synthetic slice. Topic preservation
(BERTopic cross-corpus alignment) serves as the deterministic
verifier component *R_D*, ensuring semantic fidelity and enabling
a perfect-ground-truth RL corpus for structured-output learning.
This closed loop operationalizes the knowledge-gradient principle
of the philosophy chapter and supplies the verifiable reward
signal for GRPO training.

With the full catalog now operational (522 verbalized templates,
97 % coverage) and the verifier locked at weights
`{a, b, c} = {0.50, 0.05, 0.45}` (C1 AUC 0.9956, mean separation
0.336), the pipeline provides one of the cleanest verifiable-reward
setups for structured-output learning available, directly
supporting the hierarchical sequence model (H-Net dynamic chunking
front end + RWKV-7 recurrent backbone) and the downstream
byte-level pretraining slice. The remaining subsections describe
the catalog (§ The catalog), the verifier components (§ The
verifier), and the reward-signal flow into GRPO (§ Verifier signal
flow) in detail; readers tracking the executive narrative may skip
to *Appendix A* for the operational status.
